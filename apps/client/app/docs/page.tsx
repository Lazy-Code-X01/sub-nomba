...
function ResourceGroup({ title, endpoints }: { title: string; endpoints: Endpoint[] }) {
  return (
    <div className="mb-10">
...
const PLANS: Endpoint[] = [
  ...
  "isActive": false              // optional — set false to soft-deactivate
}`,
  },
  {
    method: "DELETE",
...
// CREATED | TRIALING | ACTIVE | PAST_DUE | PAUSED | CANCELLED`,
  },
  {
    method: "POST",
    path: "/api/v1/subscriptions",
    desc: "Create a subscription linking a customer to a plan. Status starts as TRIALING if the plan has trialDays > 0, otherwise CREATED. Call /bill-now after creation to generate the first invoice and collect payment.",
    body: `{
  ...
  },
  {
    ...
    desc: "Generate a PENDING invoice for this subscription on demand. Returns the invoice — pass the invoice id to POST /invoices/:id/checkout to get a Nomba payment link. Once paid, Sub automatically activates the subscription.",
    note: "Required step to activate a CREATED subscription. Plans with trialDays: 0 start as CREATED and need a successful payment to reach ACTIVE.",
  },
  {
    method: "POST",
  ...
  },
  {
    ...
    desc: "Upgrade or downgrade to a different plan on an ACTIVE subscription. Proration is calculated automatically and included in the subscription.plan_changed webhook payload.",
    body: `{
...
const INVOICES: Endpoint[] = [
  ...
  },
  {
    ...
    desc: "Void a PENDING invoice. Cannot be called on an invoice that is already PAID.",
  },
];
...
const WEBHOOK_EVENTS_ENDPOINT: Endpoint[] = [
  {
    ...
    desc: "List all outbound webhook delivery attempts for your tenant — event type, status (PENDING | DELIVERED | FAILED), attempt count, timestamps, and full payload.",
  },
];
...
export default function DocsPage() {
    ...
    <main className="min-h-screen bg-canvas font-sans antialiased">
      ...
      <nav className="fixed top-0 inset-x-0 z-50 h-14 flex items-center justify-between px-8 bg-canvas border-b border-stroke">
        ...
        <div className="flex items-center gap-6">
          <Link href="/" className="font-mono text-[11px] text-label-3 hover:text-label transition-colors">
            Home
          </Link>
          <Link
            href="/signup"
      ...
      <div className="max-w-4xl mx-auto px-6 pt-24 pb-24">
        ...
        {/* Hero */}
        <div className="mb-16">
          <div className="font-mono text-[10px] uppercase tracking-widest text-label-3 border border-stroke rounded-full px-4 py-1.5 inline-block mb-6">
            API Reference
          </div>
          <h1 className="font-sans font-extrabold text-[42px] sm:text-[52px] leading-[1.08] tracking-[-1px] text-label mb-5">
            Build billing<br />in an afternoon
          </h1>
          <p className="font-sans text-[15px] text-label-2 leading-relaxed max-w-2xl mb-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <span className="font-mono text-[11px] text-label-3">Base URL</span>
            <code className="font-mono text-[11px] text-label-2 bg-surface-2 border border-stroke px-3 py-1.5 rounded-lg">
              {BASE}
            </code>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-16">
          {["Quick start", "Auth", "Endpoints", "Events", "Verification", "Responses"].map((s) => (
            <a
              key={s}
              href={`#${s.toLowerCase().replace(" ", "-")}`}
              className="font-mono text-[10px] uppercase tracking-widest text-label-3 border border-stroke rounded-full px-3 py-1.5 hover:text-label hover:border-stroke-2 transition-colors"
            >
              {s}
            </a>
          ))}
        ...
        <section id="auth" className="mb-16 scroll-mt-20">
          ...
          <p className="font-sans text-[13px] text-label-2 mb-5">
            ...
            Keys are scoped per tenant — data is fully isolated. Find your key in{" "}
  ...
  -H "x-api-key: sub_live_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"`}
        ...
        <section id="endpoints" className="mb-16 scroll-mt-20">
          ...
          <ResourceGroup title="Plans" endpoints={PLANS} />
          <ResourceGroup title="Customers" endpoints={CUSTOMERS} />
          <ResourceGroup title="Subscriptions" endpoints={SUBSCRIPTIONS} />
          <ResourceGroup title="Invoices" endpoints={INVOICES} />
          <ResourceGroup title="Webhook Events" endpoints={WEBHOOK_EVENTS_ENDPOINT} />
        </section>

        {/* 04 Events table */}
        <section id="events" className="mb-16 scroll-mt-20">
          <SectionLabel n="04" tag="Events" title="Webhook events" />
          <p className="font-sans text-[13px] text-label-2 mb-6">
            Sub sends a signed <Mono>POST</Mono> to your webhook URL for each event.
            Every delivery includes an <Mono>X-Sub-Signature</Mono> and <Mono>X-Sub-Timestamp</Mono> header.
            Configure your endpoint and secret in the{" "}
            <Link href="/webhooks" className="text-yellow hover:underline">Webhooks</Link> dashboard page.
          </p>
          <div className="bg-surface border border-stroke rounded-xl overflow-hidden">
            <div className="grid grid-cols-[1.6fr_2.4fr_1.4fr] gap-4 px-5 py-3 bg-surface-2 border-b border-stroke">
            ...
            {EVENTS.map((e, i) => (
              <div
                key={i}
                className={`grid grid-cols-[1.6fr_2.4fr_1.4fr] gap-4 px-5 py-3.5 items-start ${i < EVENTS.length - 1 ? "border-b border-stroke" : ""}`}
              >
        ...
        <section id="verification" className="mb-16 scroll-mt-20">
          ...
          <p className="font-sans text-[13px] text-label-2 mb-2 leading-relaxed">
            ...
            <strong className="text-label font-semibold">HMAC-SHA256</strong> over the raw JSON body using your tenant&apos;s webhook secret.
            The hex-encoded signature is sent in <Mono>X-Sub-Signature</Mono>.
            Always verify before processing.
          </p>
          <p className="font-sans text-[13px] text-label-2 mb-5 leading-relaxed">
            Make sure to use the <strong className="text-label font-semibold">raw body</strong> — not the parsed JSON object — when verifying.
          </p>
          <pre className="bg-surface border border-stroke rounded-xl px-5 py-5 font-mono text-[11px] leading-[1.8] text-label-2 overflow-x-auto whitespace-pre">
            {VERIFY_SNIPPET}
          </pre>
        ...
        <section id="responses" className="mb-16 scroll-mt-20">
          ...
          <p className="font-sans text-[13px] text-label-2 mb-5">
            All endpoints return the same JSON envelope — check <Mono>success</Mono> first, then read <Mono>data</Mono> or <Mono>message</Mono>.
          </p>
